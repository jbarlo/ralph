...
# System deps + Playwright Chromium runtime deps. patchelf is here to fix
# the ELF interpreter on the bun-compiled ralph binary if it was built on a
# host whose interpreter lives at a non-standard path (e.g. NixOS).
RUN apt-get update && apt-get install -y --no-install-recommends \
    curl xz-utils git bash ca-certificates patchelf \
...
RUN chmod +x /usr/local/bin/ralph && \
    patchelf --set-interpreter /lib64/ld-linux-x86-64.so.2 /usr/local/bin/ralph
